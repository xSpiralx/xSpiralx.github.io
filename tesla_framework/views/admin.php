...
<form method="post" action="save_options">
  ...
  <div class="tt_top_bar">
    <div class="tt_top_links">
      <span>
        <a class="tt_documentation" href="#"><?php _e("Documentation", 'teslawp')?></a>
      </span>
      <span>
        <a class="tt_support" href="#"><?php _e("Support Tickets", 'teslawp')?></a>
      </span>
      <span>
        <a class="tt_news" href="#"><?php _e("Latest News", 'teslawp')?></a>
      </span>
    </div>
    <div class="tt_theme_logo">
      <span><?php echo THEME_PRETTY_NAME ?></span><?php _e(" Version " . THEME_VERSION, 'teslawp');?> | 
      <span><?php _e("Framework ", 'teslawp')?></span><?php _e("Version " . TT_FW_VERSION, 'teslawp')?> 
    </div>
  </div>
  <div class="tt_sidebar">
    <div class="tt_logo"><a href="http://www.teslathemes.com?utm_source=FWBannerTesla&utm_medium=banner&utm_campaign=TeslaFree" target="_blank"></a></div>
    ...
    <?php foreach ( $tabs as $key => $tab ) : ?>
      ...
      ><a
            ...
            data-toggle="tab"
          ><?php printf( __( '%s',  'teslawp' ), $tab[ 'title' ] ) ?></a>
      </li>
    <?php endforeach; ?>
    </ul>
  </div>
  <div class="tt_content">
    <a class="tt_teslathemes" target="_blank" href="http://teslathemes.com/?utm_source=FWBannerTesla&utm_medium=banner&utm_campaign=TeslaFree"></a>
    <?php $j = 0 ;?>
<!-- ========================================= TABS START ========================================================== -->
    <?php foreach ( $tabs as $key => $tab ) : ?>
      <div
          ...
          <?php endif;?>
              <div class="tt_content_box">
                <div class="tt_content_box_title">
                  <span class="tt_bg_icon tt_<?php if(!empty($box['icon'])) echo $box['icon'];?>"><?php printf( __( '%s',  'teslawp' ), $box_name ); ?></span>
                </div>
                  ...
                  <?php endif;?>
                    <?php if (!empty($input_field_name) && !is_int($input_field_name)):?>
                      <div class="tt_option_title">
                        <span><?php printf( __( '%s',  'teslawp' ), $input_field_name ); ?></span>
                      </div>
                    <?php endif;?>
                    <?php 
                    $input_id = $input['id'];
                    ...
                    switch ($input['type']) : //==========================START INPUT TYPES=======================================
                            case 'select' :          //---------- SELECT------------------------------------
                                ...
                                <select
                                  ...
                                  name="<?php echo esc_attr(THEME_OPTIONS)?>[<?php echo esc_attr($input_id)?>]<?php if (!empty($input['multiple']))echo '[]';?>"
                                ...
                                <?php else:?>
                                    <?php foreach ( $input['options'] as $name=>$value ) : ?>
                                        <option
                                            value="<?php echo esc_attr($value) ?>"<?php if (!empty($input['multiple']))
                                                                                if (!empty($options[ $input_id ]))
                            ...
                            <?php case 'text' : //------------------TEXT------------------------------------?>
                                <input
                                    ...
                                    name="<?php echo esc_attr(THEME_OPTIONS)?>[<?php echo esc_attr($input_id);?>]"
                                    placeholder="<?php if (!empty($input['placeholder']))echo esc_attr($input['placeholder']);?>"
                                    value="<?php if ( ! empty( $options[ $input_id ] ) ) echo esc_attr($options[ $input_id ]) ; ?>"
                                >
                                <?php if (!empty($input['color_changer'])) : //---text color changer--------?> 
                                    <input
                                        ...
                                        class="text_color<?php if (!empty($input['class']))echo " " . esc_attr($input['class']);?>"
                                        data-default-color="<?php if (!empty($input['default']))  esc_attr($input['default']);?>" 
                                        name="<?php echo esc_attr(THEME_OPTIONS)?>[<?php echo esc_attr($input_id);?>_color]"
                                    />
                                <?php endif?>
                                <?php if (!empty($input['font_changer'])) : //---text font changer----------?>
                                    <select class="font_changer font_search" id="<?php echo esc_attr($input_id)?>_font" name="<?php echo esc_attr(THEME_OPTIONS)?>[<?php echo esc_attr($input_id)?>_font]">
                                    <?php 
                                    $fonts = get_google_fonts();
                                    if ( !empty($fonts) ) :?>
                                        <?php foreach ( $fonts->items as $font ) : ?>
                                            <option value='<?php echo esc_attr($font->family) ?>'<?php if ( $font->family == $options[ $input_id . "_font" ]  ) echo ' selected="selected"'  ?>><?php echo $font->family ?></option>
                                        <?php endforeach; ?>
                                    <?php endif;?>
                                    </select>
                                <?php endif;?>
                                <?php if (!empty($input['font_size_changer'])) : //---text size changer-----?>
                                    <input
                                      ...
                                      name="<?php echo esc_attr(THEME_OPTIONS)?>[<?php echo esc_attr($input_id)?>_size]"
                                      data-size-unit ="<?php echo esc_attr($input['font_size_changer'][2]);?>"
                                      ><span class="units"><?php if (!empty($input['font_size_changer'][2])) echo esc_attr($input['font_size_changer'][2])?></span>
                                <?php endif;?>
                                <?php if (!empty($input['font_preview'][0])) : //---Font Preview---------------?>
                                    <div class="tt_option_title mt30"><span><?php _e('Text Logo Preview', 'teslawp')?></span></div>
                                    <div
                            ...
                            <label class="tt_checkbox tt_checkbox_<?php if(!empty($input['list'])) echo 'list'; else echo 'grid'?> <?php if (!empty($input['class']))echo $input['class']?>">
                                <input
                                    ...
                                    <?php if (!empty($input['action'])) :?>
                                      data-tt-interact-objs='<?php echo esc_attr(json_encode($input['action'][1])) ?>'
                            ...
                            <?php case 'colorpicker' : //------------------COLORPICKER----------------------?>
                                <input
                                    ...
                                    type="text" 
                                    value="<?php if ( ! empty( $options[ $input_id ] ) ) echo esc_attr($options[ $input_id ]); ?>"
                                    class="my-color-field<?php if (!empty($input['class']))echo " " . esc_attr($input['class']);?>"
                            ...
                            <?php case 'textarea' : //------------------TEXTAREA----------------------------?>
                                ...
                                ><?php if ( ! empty( $options[ $input_id ] ) )
                                          echo ( esc_textarea($options[ $input_id ]) );
                                       elseif (! empty( $input['default_value'] ) )
                                          echo ( esc_textarea($input['default_value']) );
                                  ?></textarea>
                            <?php break;?>
                            <?php case 'image_upload' ://------------------IMAGE UPLOAD---------------------?>
                                    ...
                                    <?php
                                        if ( !empty( $input ['title'] ) )
                                          printf( __( '%s',  'teslawp' ), $input ['title'] );
                                        else
                                          _e('Upload Image',  'teslawp');?>
                                    </button>
                                    <button class='tt_button remove_img'><?php _e('Remove Image', 'teslawp')?></button>
                                    
                                    <div class="tt_option_title"><span><?php _e("Preview", 'teslawp')?></span></div> 
                            ...
                            <?php case 'date_picker' ://------------------Date Picker-----------------------?>
                                ...
                                <?php if (!empty($input['date_range'])) : //---Date Range-------------------?> 
                                    <input
                                        class="datepicker"
                                        id="<?php echo esc_attr($input_id) . '-to';?>"
                                        type="text"
                                        name="<?php echo esc_attr(THEME_OPTIONS)?>[<?php echo esc_attr($input_id) . '-to';?>]"
                                        value="<?php if ( ! empty( $options[ $input_id . '-to' ] ) ) echo  esc_attr($options[ $input_id . '-to' ]) ; ?>"
                                    />
                                <?php endif?>
                            <?php break;?>
                            <?php case 'button' ://------------------BUTTON---------------------------------?>
                                <button
                                    ...
                                    name="<?php echo esc_attr(THEME_OPTIONS)?>[<?php echo esc_attr($input_id);?>]"
                                ><?php if ( ! empty(  $input [ 'value' ] ) ) printf(__( '%s',  'teslawp' ),$input['value']); ; ?></button>
                            <?php break;?>
                                  ...
                                  foreach ( $input['platforms'] as $platform ) : ?>
                                    <li>
                                      <input
                                        ...
                                        name="<?php echo esc_attr(THEME_OPTIONS)?>[<?php echo esc_attr($input_id . "_" . $platform)?>]"
                                        placeholder="<?php echo esc_attr(ucfirst($platform))?>">
                                    </li>
                                  <?php endforeach;?>
                              </ul>
                            <?php break;?>
                            <?php case 'map' ://------------------MAP-------------------------------------?>
                                ...
                                <div
                                  id="map-canvas"
                                  class="tt_map <?php if ( ! empty( $input['class' ] ) ) echo esc_attr($input['class']) ; ?>"
                                  name="<?php echo esc_attr(THEME_OPTIONS)?>[<?php echo esc_attr($input_id);?>]"
                                ></div>
                                <input
                                  id="map-coords"
                                  type="hidden"
                                  name="<?php echo esc_attr(THEME_OPTIONS)?>[<?php echo esc_attr($input_id);?>_coords]"
                                  value="<?php if ( ! empty( $options[ $input_id . "_coords" ] ) ) echo esc_attr($options[ $input_id . "_coords" ]) ;?>">
                                <input
                                  id="marker-coords"
                                  type="hidden"
                                  name="<?php echo esc_attr(THEME_OPTIONS)?>[<?php echo esc_attr($input_id);?>_marker_coords]"
                                  value="<?php if ( ! empty( $options[ $input_id . "_marker_coords" ] ) ) echo esc_attr($options[ $input_id . "_marker_coords" ]) ;?>">
                                <input
                                  id="map-zoom"
                                  type="hidden"
                                  name="<?php echo esc_attr(THEME_OPTIONS)?>[<?php echo esc_attr($input_id);?>_zoom]"
                                ...
                                <?php if (!empty($input['icons']))
                                  foreach ($input['icons'] as $icon) : ?>
                                    <label class="map-icon">
                                      <input
                                        type="radio"
                                        name="<?php echo esc_attr(THEME_OPTIONS)?>[<?php echo esc_attr($input_id);?>_icon]"
                                        value="<?php echo esc_attr(TT_FW . '/static/images/mapicons/' . $icon)?>"
                                        <?php if(!empty($options[ $input_id . '_icon']))checked( TT_FW . '/static/images/mapicons/' . $icon , $options[ $input_id . '_icon']); ?>
                                        ><img src="<?php echo esc_attr(TT_FW . '/static/images/mapicons/' . $icon)?>" alt="map icon" /></label>
                                  <?php endforeach;?>
                            <?php break;?>
                ...
                <div class="tt_content_box_bottom">
                  <input class="tt_submit" type="submit" value="<?php _e('Save Changes', 'teslawp')?>">
                    <div class="tt_bottom_note">
                      <?php printf( __( '%s',  'teslawp' ), $option_saved_text ); ?>
                    </div>
                </div>
              </div>
            <?php if (count($tab[ 'boxes' ]) > 1):?>
